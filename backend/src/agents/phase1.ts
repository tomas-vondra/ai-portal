...
export async function runPhase1(ctx: AgentContext): Promise<Record<string, unknown>> {
  ...
  const sources = (input?.sources as string[]) ?? ['Google', 'LinkedIn', 'ARES'];

  await ctx.publishLog('ok', `Prohledávám zdroje pro: ${companyName}`);

  // TODO: Real integrations with Google Search API, ARES, LinkedIn
  // For now, we use LLM to generate the research report
